...
<body>
	<div class="layout">
		<table>
			...
			<tbody>
			<h5 align="right">글 개수: ${listcount} </h5>
		...
		<nav aria-label="Page navigation example">
			<ul>
				<c:if test="${startPage > 10 }">
					<a href="reviewList.do?page=${startPage+1 }">이전</a>
				</c:if>
				<c:forEach begin="${startPage}" end="${endPage}" var="pageNum">
					<a href="reviewList.do?page=${pageNum}">${pageNum }</a>
				</c:forEach>
				<c:if test="${endPage < pageCount}">
					<a href="reviewList.do?page=${endPage + 1}">다음</a>
				</c:if>
			</ul>
		</nav>
	</div>
